...
class CrawlJob:
    ...
    company:      str               # 회사명 (company_id는 Orchestrator가 DB에서 조회/생성)
    title:        str               # 공고 제목
    content:      str               # 공고 내용
    region:       str               # 근무지
    job_type:     str               # 직무
    external_url: str               # 채용공고 원본 URL
    source_type:  SourceType        # 출처 (각 크롤러 구현체가 고정값으로 설정)
    status:       JobStatus = field(default=JobStatus.OPEN)  # 크롤링 시점엔 항상 모집중
